const View = {
    table: {
        __generateDTRow(data){ 
            return [
                ...
                ViewIndex.table.formatNumber(data.discount),
                data.discount_time,
    ...
    modals: {
        ...
        Create: {
            ...
            setDefaul(){ this.init();  },
            setVal(data){ 
                $(".data-product option").remove()
                data.map(v => {
                    $(".data-product").append(`<option value="${v.id}" data-prices="${v.prices}">${v.name}</option>`)
                })
                $(".prices-defaul").val(View.formatNumber($(".data-product option:selected").attr("data-prices")))
            },
            getVal(){
